...
## $${\color{Red} Warning \space : \space Second \space Beta \space Release \space 3.0.1 \space }$$
The current release (v3.0.1beta2) available is undergoing final operations testing. It has passed all unit tests and ad hoc use case testing but still requires a final review.
...
## $${\color{Red} Avis \space : Deuxième \space version \space bêta \space 3.0.1 \space }$$
La version actuelle (v3.0.1beta2) disponible fait l'objet de tests opérationnels finaux. Elle a passé avec succès tous les tests unitaires et les tests d'intégration, mais doit encore faire l'objet d'un dernier cycle d'assurance qualité.

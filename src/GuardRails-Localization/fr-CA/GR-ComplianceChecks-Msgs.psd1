...
keyVaultCertValidationFailed = Le certificat stocké dans Key Vault pour l'écouteur « {0} » dans l'Application Gateway « {1} » n'a pas pu être validé. La solution CC nécessite les autorisations « Key Vault Secrets User» sur le Key Vault du client pour valider les certificats. Contactez votre administrateur pour accorder au compte d'automatisation CC (ID : {2}) l'accès à ce Key Vault si vous le souhaitez.

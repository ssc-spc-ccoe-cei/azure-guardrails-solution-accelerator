...
noMFAPolicyForAllUsers = Aucune politique d'accès conditionnel exigeant l'authentification multifactorielle (MFA) pour tous les utilisateurs et toutes les applications n'a été trouvée. Une politique d'accès conditionnel répondant aux exigences suivantes doit être configurée : 1. state = «activé»; 2. includedUsers = «All»; 3. includedApplications = «All»; 4. grantControls.builtInControls contient «mfa» OU grantControls.authenticationStrength est configuré; 5. clientAppTypes contient «all» ou tous les types individuels sélectionnés (browser, mobileAppsAndDesktopClients, exchangeActiveSync, other); 6. userRiskLevels = @(); 7. signInRiskLevels = @(); 8. platforms = null; 9. locations = null; 10. devices = null; 11. clientApplications = null
...
# GuardRail #9
ddosPublicIpCheckFailed = Impossible de vérifier la protection DDoS des adresses IP publiques pour l'abonnement '{0}'. Erreur : {1}
ddosPublicIpCheckFailedNoProtection = La protection DDoS des adresses IP publiques n'a pas pu être vérifiée pour l'abonnement '{0}'. {1} VNet(s) inclus sur {2} ont la protection DDoS activée. Les résultats des adresses IP publiques sont indisponibles.
ddosSubscriptionNoResources = L'abonnement '{0}' n'a pas de VNet inclus ni d'adresse IP publique à évaluer.
ddosSubscriptionProtectionFound = L'abonnement '{0}' a la protection DDoS activée sur {1} VNet(s) inclus sur {2} et {3} adresse(s) IP publiques sur {4}.
ddosSubscriptionProtectionMissing = L'abonnement '{0}' n'a pas la protection DDoS activée sur un VNet inclus ou une adresse IP publique. VNet(s) inclus protégés : {1} sur {2}. Adresses IP publiques protégées : {3} sur {4}.
subscriptionScope = Portée de l'abonnement
...
vnetDDosConfig = Configuration DDoS - Abonnement
...
NoEnabledHealthAlert = Les alertes de santé du service « Service Health Alerts » ne sont pas activées pour cet abonnement. Assurez-vous que les alertes d'état du service sont configurées sur cet abonnement et que le groupe d'action associé à l'alerte a au moins deux contacts différents.
...
NotAllSubsHaveDefenderPlans = L'abonnement n'a pas de plan « Defender ». Activez la surveillance « Defender » pour cet abonnement.

...
function Write-GuardrailTelemetry {
    ...
    try {
        ...
        if ($null -ne $DurationMs) { $record['DurationMs_d'] = [Math]::Round($DurationMs, 2) }
        if ($null -ne $ErrorCount) { $record['ErrorCount_d'] = [double]$ErrorCount }
        if ($null -ne $WarningCount) { $record['WarningCount_d'] = [double]$WarningCount }
        if ($null -ne $ItemCount) { $record['ItemCount_d'] = [double]$ItemCount }
        if ($null -ne $CompliantCount) { $record['CompliantCount_d'] = [double]$CompliantCount }
        if ($null -ne $NonCompliantCount) { $record['NonCompliantCount_d'] = [double]$NonCompliantCount }
        if ($ReportTime) { $record['ReportTime_s'] = $ReportTime }
        if (-not [string]::IsNullOrWhiteSpace($Message)) { $record['Message_s'] = $Message }
...
#end Guardrail Telemetry Helpers

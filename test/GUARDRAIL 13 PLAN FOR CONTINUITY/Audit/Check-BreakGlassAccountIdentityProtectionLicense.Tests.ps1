...
Describe 'Get-BreakGlassAccountLicense' {
    ...
    It 'Returns compliant when both break glass accounts have AAD Premium P2' {
        Mock Invoke-GraphQueryEX -ModuleName Check-BreakGlassAccountIdentityProtectionLicense {
            param($urlPath)
            switch ($urlPath) {
                '/users/bg1@contoso.com' { [pscustomobject]@{ Content = [pscustomobject]@{ value = [pscustomobject]@{ id = '1' } } } ; break }
                '/users/bg2@contoso.com' { [pscustomobject]@{ Content = [pscustomobject]@{ value = [pscustomobject]@{ id = '2' } } } ; break }
                '/users/bg1@contoso.com/licenseDetails' { [pscustomobject]@{ Content = [pscustomobject]@{ value = ,([pscustomobject]@{ servicePlans = @([pscustomobject]@{ ServicePlanName = 'AAD_PREMIUM_P2' }) }) } } ; break }
                '/users/bg2@contoso.com/licenseDetails' { [pscustomobject]@{ Content = [pscustomobject]@{ value = ,([pscustomobject]@{ servicePlans = @([pscustomobject]@{ ServicePlanName = 'AAD_PREMIUM_P2' }) }) } } ; break }
            }
        }
    ...
    It 'Returns non-compliant when one account lacks the license' {
        Mock Invoke-GraphQueryEX -ModuleName Check-BreakGlassAccountIdentityProtectionLicense {
            ...
            switch ($urlPath) {
                '/users/bg1@contoso.com' { [pscustomobject]@{ Content = [pscustomobject]@{ value = [pscustomobject]@{ id = '1' } } } ; break }
                '/users/bg2@contoso.com' { [pscustomobject]@{ Content = [pscustomobject]@{ value = [pscustomobject]@{ id = '2' } } } ; break }
                '/users/bg1@contoso.com/licenseDetails' { [pscustomobject]@{ Content = [pscustomobject]@{ value = @() } } ; break }
                '/users/bg2@contoso.com/licenseDetails' { [pscustomobject]@{ Content = [pscustomobject]@{ value = ,([pscustomobject]@{ servicePlans = @([pscustomobject]@{ ServicePlanName = 'AAD_PREMIUM_P2' }) }) } } ; break }
            }
        }

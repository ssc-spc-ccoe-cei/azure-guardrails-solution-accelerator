...
.DESCRIPTION
    ...
    context with the operator, downloads the requested GitHub branch, tag, or commit, imports the downloaded Guardrails setup module, and then calls
    ...
    Adding new components during a fresh install is supported with -configFile.
    Adding new components to an existing deployment is supported with -keyVault.
...
.PARAMETER configFile
    ...
    Existing-deployment component-addition flows should use -keyVault instead so the saved deployment identity can be preserved.
.PARAMETER keyVault
    ...
    or existing-deployment new-components flows.
.PARAMETER source
    Optional GitHub branch, tag, or commit to download and run. If omitted, the bootstrap script
    resolves the latest GitHub release tag and uses that as the source.
.PARAMETER tagsFile
    Optional path to a tags.json file to use instead of the downloaded GitHub branch or tag copy's default setup/tags.json.
    ...
    In hardened flows, the reserved mandatory Azure resource tags (Solution, ReleaseVersion, ReleaseDate)
    still come from the selected GitHub source or the saved deployment record in Key Vault.
    Use -tagsFile for custom Azure resource tags only.
.PARAMETER update
    Update an existing deployment.
.PARAMETER newComponents
    New components to deploy. With -configFile, this adds components during a fresh install.
    With -keyVault, this must be used for existing-deployment component-addition flows.
...
.EXAMPLE
    # If -source is omitted, bootstrap resolves the latest GitHub release automatically.
    ./Deploy-Bootstrap.ps1 -configFile ./config.json
.EXAMPLE
...
.EXAMPLE
    # Apply client-specific custom tags before a fresh install. Reserved mandatory tags still come from the
    # effective release source.
...
.EXAMPLE
    # Apply client-specific custom tags before an update. Reserved mandatory tags still come from the
    # effective release source or saved deployment tags.
...
# Download the requested GitHub branch or tag as a zip file, unpack it,
...
function Get-BootstrapSourceArchive {
    ...
    try {
        ...
        # Make the GitHub branch or tag name safe to place in a web address, including names with "/".
...
# Resolve the latest GitHub release tag when the operator does not specify -source.
function Get-BootstrapLatestReleaseSource {
    try {
        $latestRelease = Invoke-RestMethod -Uri "$($script:GitHubApiRoot)/releases/latest" -Headers @{ 'User-Agent' = 'Deploy-Bootstrap' } -ErrorAction Stop -Verbose:$false
    }
    catch {
        throw "Unable to resolve the latest GitHub release for Deploy-Bootstrap.ps1. $_"
    }

    if ([string]::IsNullOrWhiteSpace($latestRelease.tag_name)) {
        throw "GitHub latest release response did not include a usable tag name."
    }

    return $latestRelease.tag_name.Trim()
}

# Check that GitHub can find the requested branch or tag and that it includes the
...
function Test-BootstrapSource {
    ...
    # Build the GitHub URL for the module zip files in this branch or tag.
    ...
    try {
        ...
        if ($statusCode -eq 403) {
            throw "GitHub returned HTTP 403 while validating source '$Source'. This may be caused by rate limiting, insufficient permissions, or the GitHub branch or tag not being accessible. $_"
        }
...
# Replace the downloaded setup/tags.json file with an operator-supplied one for custom Azure resource tags.
function Set-BootstrapDownloadedTagsFile {
    ...
    Write-Host ("Using tags file '{0}' in place of downloaded setup/tags.json for custom Azure resource tags. Reserved mandatory Azure resource tags will still be resolved by the deploy module." -f $TagsFilePath)
}
...
try {
    ...
    # If the operator does not specify -source, use the latest GitHub release tag.
    if ([string]::IsNullOrWhiteSpace($source)) {
        $source = Get-BootstrapLatestReleaseSource
        Write-Host ("No -source was specified. Using latest GitHub release '{0}'." -f $source)
    }
    else {
        $source = $source.Trim()
    }
    ...
    if (Get-Command -Name 'bicep' -ErrorAction SilentlyContinue) {
        $detectedBicepVersion = (& bicep --version 2>$null).Trim()
        $bicepSummary = if ($detectedBicepVersion) { $detectedBicepVersion } else { 'detected' }
    }
    ...
    # Check the requested GitHub branch or tag before asking the operator to continue.
    ...
    Write-Host "  Mandatory Azure resource tags: resolved by the downloaded deploy logic from the selected GitHub source or saved deployment tags in Key Vault"
    if ($PSBoundParameters.ContainsKey('tagsFile')) {
        Write-Host ("  Tags file: {0} (custom Azure resource tags; reserved mandatory Azure resource tags are resolved separately)" -f $tagsFile)
    }
    else {
